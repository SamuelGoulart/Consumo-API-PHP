...
const createContact = async (contato) => {
    const url = "http://localhost/apiphp/backend/contatos/"
    const options = {
        method: 'POST',
...
const createRow = (contato) => {
    ...
    newRow.innerHTML = `
        ...
        <td>
            <button type='button' class="btn btn-warning" data-action="editar-${contato.id}">Editar</button>
            <button type='button' class="btn btn-danger" data-action="deletar-${contato.id}">Deletar</button>
        </td>
    `
